...
public:
    ...
    virtual int sendTo(const iovec *iov, size_t iovcnt, const Address::ptr to, int flags = 0) override;
    virtual int recvFrom(void *buffer, size_t length, Address::ptr from, int flags = 0) override;
    virtual int recvFrom(iovec *ba, size_t length, Address::ptr from, int flags = 0) override;
